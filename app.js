...
function restoreQueue(){
 try{
  ...
  if(data.version!==1||!Array.isArray(data.queue)||data.queue.length>24||data.queue.some(id=>!Number.isInteger(id)||id<1||id>24)||new Set(data.queue).size!==data.queue.length)throw Error('Invalid saved queue');
...
function saveQueue(){
 try{
  localStorage.setItem(storageKey,JSON.stringify({version:1,queue}));
  saved=true;
  storageNotice('この端末に自動保存済み。再読み込み後も提供順を復元します。');
 }catch{
  saved=false;
...
function addSeat(id){if(!seats.has(id))throw Error('席番号が正しくありません');if(queue.includes(id))throw Error('受付済みです');queue.push(id);saveQueue();render();announce(label(id)+'を '+queue.length+'番で受け付けました');return {seat:id,order:queue.length}}
...
function completeSeat(id){const index=queue.indexOf(id);if(index<0)throw Error('提供待ちの席ではありません');queue.splice(index,1);saveQueue();render();announce(label(id)+'への提供が完了しました');return {seat:id,waiting:queue.length}}
...
restoreQueue();
render();
if(queue.length)announce('保存していた '+queue.length+'席の提供順を復元しました');
